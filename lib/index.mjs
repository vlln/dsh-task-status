...
function collectTasks(ctx) {
	...
	const out = [];
	for (const agent of ctx.agents.list()) for (const snapshot of tasks.list(agent)) {
...
function readTaskOutput(ctx, id) {
	...
	const tasks = ctx.tasks;
	let caller;
	for (const agent of ctx.agents.list()) if (tasks.list(agent).some((snapshot) => snapshot.id === id)) {
		caller = agent;
		break;
	}
