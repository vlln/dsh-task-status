...
git clone https://github.com/dsh-external/dsh-task-status
cd dsh-task-status && pnpm install && pnpm run build

# 2. 挂载到 web profile（<包路径> = 含 dsh.bundle 且构建产物在库的目录）
dsh plugin --profile web add /path/to/dsh-task-status
```
